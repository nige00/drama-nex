...
const Actor: NextPage = () => {
  return (
    <Layout
      title={`${fixedSentence.date}U-NEXTで見れる国内ドラマの俳優・女優一覧`}
      description={`U-NEXTで見れる国内ドラマの俳優・女優一覧`}
      keyword={fixedSentence.keywords}
      url={`${fixedSentence.url}/actor`}
      type="article"
    >
      <h1 className="text-xl sm:text-2xl text-gray-600">{`${fixedSentence.date}U-NEXTで見れる国内ドラマの俳優・女優一覧【${actorList.length}人】`}</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-6 w-11/12">
        {actorList.map((content) => {
          return (
            <div key={content.actorName}>
              <Link href={`/actor/${content.pageUrl}`}>
